    ...
    >
      <template v-slot:top>
        <v-toolbar flat>
          ...
          <v-dialog v-model="dialogDelete" max-width="700px" persistent>
            <v-card>
              <v-card-title class="text-h5">Are you sure you want to delete this permission category?</v-card-title>
              <v-card-actions>
                <v-spacer></v-spacer>
                <template>
      ...
      <template v-slot:item.actions="{ item }">
        <div class="tw-flex tw-items-end tw-justify-end">
          <div class="tw-float-left tw-ml-2" v-can="'update_permission_category'">
            <v-icon small title="Edit" class="hover:tw-text-green-500" @click="editItem(item)">mdi-pencil</v-icon>
          </div>
          <div class="tw-float-left tw-ml-2" v-can="'delete_permission_category'">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      deleteItemConfirm() {
        axios.delete('http://localhost:8000/api/v1/permission-categories/' + this.editedId)
        .then(() => {
      ...
      async save () {
        ...
        if (!this.$v.$invalid) {
          if (this.editedIndex > -1) {
            await axios.patch('http://localhost:8000/api/v1/permission-categories/' + this.editedId, this.editedItem)
            .then(() => {
            ...
            axios.post('http://localhost:8000/api/v1/permission-categories', this.editedItem)
            .then((response) => {
    ...
    computed: {
      ...
      formTitle () {
        return this.editedIndex === -1 ? 'New Permission Category' : 'Edit Permission Category'
      },
